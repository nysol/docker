...
RUN yum -y install boost-devel libxml2-devel
RUN yum -y install quota
...
WORKDIR ruby-build
RUN ./install.sh
RUN yum install -y zlib-devel bzip2 bzip2-devel readline-devel sqlite sqlite-devel openssl-devel
RUN rbenv install 2.5.1; rbenv rehash; rbenv global 2.5.1;
...
WORKDIR mcmd
RUN aclocal; autoreconf -i; ./configure; make; make install;
...
#  - JUMAN
WORKDIR /work
RUN curl -O http://nlp.ist.i.kyoto-u.ac.jp/nl-resource/juman/juman-7.01.tar.bz2
RUN tar jxvf juman-7.01.tar.bz2
WORKDIR juman-7.01
RUN ./configure; make; make install;
#  - KNP
WORKDIR /work
RUN curl -O http://nlp.ist.i.kyoto-u.ac.jp/nl-resource/knp/knp-4.19.tar.bz2
